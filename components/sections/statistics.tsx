    ...
    return spring.on("change", (v) => {
      if (ref.current) {
        ref.current.textContent =
          Math.floor(v).toLocaleString("nb-NO") + suffix;
      }
    });
  }, [spring, suffix]);
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-start">
          ...
          <StaggerChildren className="flex-1 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
            {stats.map((stat) => (
              ...
              >
                <div
                  className={`absolute inset-0 ${stat.glow} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                />
                <div className="relative z-10">
                  <p
                    className={`text-4xl font-bold ${stat.color} mb-2 tabular-nums`}
                  >
          ...
          <div className="glass-border rounded-2xl bg-brand-surface p-6 flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-sm text-brand-muted">
                Tilgjengelig i Norge, Danmark og snart Sverige — nå også Finland
                på vei.
              </p>
            </div>
          </div>
