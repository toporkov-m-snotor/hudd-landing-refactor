...
function ArticleCard({ article, featured = false, readMore }: ArticleCardProps) {
  return (
    <motion.div
      variants={fadeItem}
    ...
    >
      <Link href={article.href} target="_blank" rel="noreferrer" className="block h-full">
        ...
        <div className={`p-5 ${featured ? "p-6" : ""}`}>
          ...
          <h3 className={`font-semibold text-brand-text leading-snug mb-2 ${featured ? "text-lg" : "text-sm"}`}>
            {article.title}
          </h3>
          <p className="text-sm text-brand-muted leading-relaxed line-clamp-2">{article.text}</p>
...
export function HuddInMedia() {
  const t = useTranslations("huddInMedia");
  ...
  return (
    <section className="py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <ArticleCard article={featuredArticle} featured readMore={t("readMore")} />
